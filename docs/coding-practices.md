...
## Record Type Access Pattern

Record types are defined as typed structs conforming to `MedicalRecordContent`. Each struct
declares its own `fieldMetadata` and serialization. `RecordTypeRegistry` provides a single
lookup point for display names, icons, and field metadata across all record types.

### Key Components

- **`RecordType`** — enum of all supported FHIR-based record types (e.g., `.immunization`,
  `.condition`, `.procedure`). Adding a new case is a compile error until every switch is
  updated.
- **`MedicalRecordContent`** — protocol that each typed record struct conforms to
  (e.g., `ImmunizationRecord`, `ConditionRecord`). Provides `recordType`, `fieldMetadata`,
  and Codable round-tripping via `RecordContentEnvelope`.
- **`RecordTypeRegistry`** — stateless registry that delegates to `RecordType` extensions
  and the typed structs. No per-person state, no encrypted storage, no seeding step.

### Usage

```swift
let registry = RecordTypeRegistry()
let name = registry.displayName(for: .immunization)       // "Immunization"
let icon = registry.iconSystemName(for: .immunization)     // SF Symbol name
let fields = registry.fieldMetadata(for: .immunization)    // [FieldMetadata]
```
