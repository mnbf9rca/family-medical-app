...
struct CustomSchemaEvolutionBreakingTests {
    ...
    // MARK: - Required Field Change Tests

    @Test
    func save_optionalToRequired_throwsError() async throws {
        let repo = makeRepository()
        let schema = try RecordSchema(
            id: "test-schema",
            displayName: "Test Schema",
        ...
        await #expect(throws: RepositoryError.requiredFieldChangeNotAllowed(fieldId: "optional-field")) {
            try await repo.save(updatedSchema, primaryKey: testPrimaryKey)
        }
    }
}
